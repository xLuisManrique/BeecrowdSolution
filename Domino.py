...
# Essa figura permite ver claramente quantas peças haveria num jogo de dominó,
# por exemplo, do tipo duplo-4: seriam todas as peças das 5 primeiras linhas, 15 peças no total.
# Também poderíamos ver, seguindo o padrão da figura, quantas peças possui o jogo de dominó conhecido como mexicano,
# que é o duplo-12. Seriam 91 peças, correspondendo a 13 linhas. Para a nossa sorte, existe uma fórmula com a qual
# podemos calcular facilmente o número de peças de um jogo do tipo duplo-N,
# para um número N natural qualquer: ((N+1)*(N+2))/2. Neste problema, estamos precisando da sua ajuda para escrever
# um programa que, dado o valor N, use esta fórmula para calcular e imprimir quantas peças existem
# num jogo de dominó do tipo duplo-N.
...
valor = int(input())
print(int((valor + 1) * (valor + 2) / 2))

print(sum([x for x in range(int(input())+2)]))
